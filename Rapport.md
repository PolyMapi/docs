<h1 align="center">Rapport Projet Mapillary - RTK</h1>

Porteur de projet : PALIX Nicolas

Auteurs : FODOR Gergely - CONJARD Samuel - JARDIN Baptiste - KACHA Tom
...
# Sommaire
 ...
 - [Outils](#outils)
 - [Métriques logicielles](#metriques)
...
Comme son nom l’indique, cette Task a pour but de prendre des captures avec la caméra, elle utilise les méthodes de la classe CameraAPI afin de prendre des photos à intervalle de temps régulier. Les références de ces photos sont conservées dans la base de données afin de les télécharger par la suite.

### DownloadTask

Suite à l’utilisation de CaptureTask, nous obtenons un tableau contenant les références
des images capturées. Cette Task sert à télécharger ces images à partir de leurs références. Les images sont ainsi téléchargées dans un dossier interne à l’application et non dans la galerie photo du téléphone. Ce choix est le plus simple car cela ne pose pas de problème de permissions et parce que l’utilisateur ne veut pas forcément avoir une multitude de photos 360 degrés sur son appareil. Cette Task va également modifier la latitude et la longitude directement dans les meta-données de la photo en utilisant les coordonnées enregistré par la GpsTask. Enfin, cette Task conserve un tableau contenant les chemins de toutes les images téléchargées.
...
[Repository contenant le manuel du produit RTK et le micrologiciel qui s'exécute sur la gamme de produits SparkFun RTK](https://github.com/sparkfun/SparkFun_RTK_Firmware)
